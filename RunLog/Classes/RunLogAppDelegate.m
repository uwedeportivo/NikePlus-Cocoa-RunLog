...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
  ...
  [CDMURLFetcher fetch:pageRequest completionHandler:^(NSData *data, NSError *error) {
    if (error == nil) {
      ...
      error = nil;      
      NSLog(@"startTime = %@",CDMParseDateString([xmlDoc textAtTag:@"startTime" error:&error]));
...
- (NSPersistentStoreCoordinator *) persistentStoreCoordinator {
    ...
    NSURL *url = [NSURL fileURLWithPath: [applicationSupportDirectory stringByAppendingPathComponent: @"runs"]];
